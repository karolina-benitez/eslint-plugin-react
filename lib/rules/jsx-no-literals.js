...
module.exports = {
  ...
  create(context) {
    ...
    return {
      ...
      JSXAttribute(node) {
        let isNodeValueString;
        if (context && context.options[0] && context.options[0].allowedStrings !== undefined) {
          isNodeValueString = node.value && node.value && node.value.type === 'Literal' && typeof node.value.value === 'string' && context.options[0].allowedStrings.indexOf(node.value) !== -1;
        } else {
          isNodeValueString = node.value && node.value && node.value.type === 'Literal' && typeof node.value.value === 'string';
        }
